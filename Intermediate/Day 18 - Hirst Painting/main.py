...
# colorgram extracts a list of rgb tuple colors from a given image.
...
def set_random_color():
    """Sets the color of the turtle object to a randomly selected color extracted from the image."""
    new_color = random.choice(rgb_colors)
    timmy.color(new_color)


def draw_dots():
    """draws 10 dots horizontally on the screen."""
    for _ in range(10):
        timmy.pendown()
        set_random_color()
        timmy.dot(20)
